...
<form class="form-horizontal" method="POST" action="<?php e::url("/leader/problem/edit/{$problem->problem_id}");?>">
<fieldset>
            ...
            <script type="text/javascript">
                ...
                $(function() {
                    $('#file_upload').uploadify({
                        ...
                        'uploader' : '<?php e::url("leader/problem/upload/{$problem->problem_id}");?>',
                ...
                function add_data_item(filename, id)
                {
                    if ( $('#'+ id + ':contains("' + filename +'")').length == 0 ) {
                        var ahtml = '<li><a target="_blank" href="<?php e::url("/leader/problem/showdata/{$problem->problem_id}/");?>/' + filename + '">' + filename
                            + '</a> | <a href="<?php e::url("/leader/problem/deldata/{$problem->problem_id}/");?>/' + filename + '" class="make-sure"><?php echo(__('admin.problem.edit.delete')); ?></a></li>';
                        $('#' + id).append($(ahtml));
                    }
                    $('.make-sure').click(function(){
                        return confirm('ARE U SURE ???')
        ...
        <div class="col-sm-10">
            <ul class="list-unstyled" id="out-data">
                <?php foreach($problem->out_data_files() as $f):?>
                    <li>
                        <a target="_blank" href="<?php e::url("/leader/problem/showdata/{$problem->problem_id}/{$f}");?>"><?php echo $f;?></a>
                        | <a href="<?php e::url("/leader/problem/deldata/{$problem->problem_id}/{$f}");?>" class="make-sure"><?php echo(__('admin.problem.edit.delete')); ?></a>
                    </li>
                <?php endforeach;?>
            </ul>
        </div>
